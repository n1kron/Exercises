...
class OnboardingViewController: UIViewController, UIScrollViewDelegate {
    ...
    //let textHeaderLabel = Utiles().getPrefferedLocale() == Locale(identifier: "ru-US") ? ["Получи доступ", "Попробуй"] : ["Get access", "Try"]
    let textLitleLabel = Utiles().getPrefferedLocale() == Locale(identifier: "ru-US") ? ["Получи доступ к нашим специально разработанным упражнениям", "Попробуй нашу уникальную фитнес диету расписанную на каждый день"] : ["Get access to our specially designed exercises.", "Try our unique fitness diet for every day."]
    var textButton = Utiles().getPrefferedLocale() == Locale(identifier: "ru-US") ? ["Далее", "Далее"] : ["Next", "Next"]
    ...
    func settings(number: Int, multiplier: CGFloat, view: UIView) {
        let litleLabel = UILabel(frame: CGRect(x: 0, y: -30 * multiplier, width: view.frame.width, height: 80 * multiplier))
        settingsLabel(label: litleLabel, text: textLitleLabel[number], nameFont: "Cochin-Bold", size: 35 * multiplier, color: .white)
    ...
    func settingPurchaseView(number: Int, multiplier: CGFloat, view: UIView) {
        ...
        textView.font = UIFont.init(name: "HelveticaNeue", size: 10 * multiplier)
        ...
        buttonPurchase.setTitle(Utiles().getPrefferedLocale() == Locale(identifier: "ru-US") ? "3 дня бесплатно, после $ в неделю" : "3 days free after $ per week" , for: .normal)
        ...
        settingsLabel(label: buttonPurchase.titleLabel!, text: "", nameFont: "Cochin-Bold", size: 20 * multiplier, color: .white)
        ...
        var text =  Utiles().getPrefferedLocale() == Locale(identifier: "ru-US") ? "к нашим специально разработанным упражнениям \nк уникальной фитнес диете, которая расписана на каждый день" : "to our specially designed exercises\nto a unique fitness diet for every day"
